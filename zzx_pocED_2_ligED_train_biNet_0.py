...
def load_npy_as_voxelgrid_rgba(volume, threshold=0.0, cmap_name="coolwarm"):
    """
    把单个 volume 转 voxel 网格 + RGBA
    - 颜色和透明度都随 voxel 数值变化
    - cmap_name: 颜色映射
    """
    mask = volume > threshold
    points = np.argwhere(mask)  # (N,3) z,y,x
    values = volume[mask]

    if len(points) == 0:
        return None

    points = points[:, [2, 1, 0]]  # 调整为 (x,y,z)
    ...
    rgba = (cmap(normed) * 255).astype(np.uint8)  # (N,4) uint8
...
def volume_render_all_channels_voxel(fmap, channels=-1, threshold=0.0, voxel_size=0.6, cmap_name="coolwarm"):
    ...
    for idx, c in enumerate(channels_to_plot):
        ...
        cloud = load_npy_as_voxelgrid_rgba(volume, threshold, cmap_name=cmap_name)
        ...
        if cloud is not None:
            ...
            # --- 添加 color bar ---
            vmin, vmax = np.percentile(volume[volume>threshold], [1, 99])
            pl.add_mesh(glyph_mesh, scalars="rgba", rgba=True, opacity=0.8, clim=[vmin, vmax])
